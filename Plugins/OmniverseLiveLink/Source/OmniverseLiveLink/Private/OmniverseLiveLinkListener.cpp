...
DEFINE_LOG_CATEGORY_STATIC(OmniverseLiveLinkListener, Log, All);
...
void FOmniverseLiveLinkListener::ProcessAnimationData(const TSharedPtr<FJsonObject>& DataObject, const FName& InSubjectName)
{
	...
	if (bCreateSubject)
	{
		UE_LOG(OmniverseLiveLinkListener, Log, TEXT("Creating subject '%s'"), *InSubjectName.ToString());
	...
	if (BoneArray) // valid bone need transforms
	{
		...
		UE_LOG(OmniverseLiveLinkListener, Log, TEXT("Bone Array '%d'"), BoneArray->Num());
