...
DEFINE_LOG_CATEGORY_STATIC(OmniverseBaseListener, Log, All);
...
void FOmniverseBaseListener::OnRawDataReceived(const uint8* InReceivedData, int32 InReceivedSize)
{
	...
	while (true)
	{
		if (!DataSizeInHeader.IsSet())
		{
			...
			{
				//UE_LOG(OmniverseLiveLink, Warning, TEXT("Incomplete Data: %d"), ReceivedSize);
		...
		else if (ReceivedSize == DataSizeInHeader.GetValue())
		{
			...
			//UE_LOG(OmniverseLiveLink, Warning, TEXT("Complete package, ready to parse: %d, %d"), ReceivedSize, ReceivedData.Num());
			break;
		}
		...
		else if (ReceivedSize < DataSizeInHeader.GetValue())
		{
			...
			//UE_LOG(OmniverseLiveLink, Warning, TEXT("Incomplete package, complete next time: %d, %d"), ReceivedSize, ReceivedData.Num());
			break;
		}
		...
		else if (ReceivedSize > DataSizeInHeader.GetValue())
		{
			...
			//UE_LOG(OmniverseLiveLink, Warning, TEXT("Mixing data, need to be separated: %d, %d"), ReceivedSize, ReceivedData.Num());
		}
	}
}
