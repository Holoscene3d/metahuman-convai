...
DEFINE_LOG_CATEGORY_STATIC(OmniverseWaveStreamer, Log, All);
...
void FOmniverseWaveStreamer::ParseWave(const uint8* InReceivedData, int32 InReceivedSize)
{
	...
	if (IsHeaderPackage(InReceivedData, InReceivedSize))
	{
		...
		UE_LOG(OmniverseWaveStreamer, Warning, TEXT("Received wave format info: '%s'"), *ReceievedString);
	...
	{
		//UE_LOG(OmniverseWaveStreamer, Warning, TEXT("Wav bytes received: %i"), ReceivedData.Num());
